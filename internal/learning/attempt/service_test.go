...
func TestServiceCreatesFrozenStarterAndEnforcesOwner(t *testing.T) {
	...
	created, err := service.Create(context.Background(), CreateInput{
		LearnerID: "owner", ActivityID: "assessment-check-config", ActivityVersion: 5,
	})
	if err != nil {
		t.Fatal(err)
	}
	if created.ReleaseID != "m1-first-slice-v27" || created.ActivityHash == "" || created.TaskHash == "" || len(created.CapabilityRefs) != 3 {
...
func TestServiceRequiresReviewItemClaimForReviewActivity(t *testing.T) {
	...
	_, err := service.Create(context.Background(), CreateInput{
		LearnerID: "owner", ActivityID: "review-check-config-variant", ActivityVersion: 4,
	})
...
func TestServiceSavesCompleteWorkspaceWithRevisionCAS(t *testing.T) {
	service, _ := newTestService(t)
	created, err := service.Create(context.Background(), CreateInput{LearnerID: "owner", ActivityID: "assessment-check-config", ActivityVersion: 5})
	if err != nil {
		t.Fatal(err)
	}
