...
当前 `m1-first-slice-v31` 已发布 M1-01 至 M1-14、M2-01 至 M2-13。学习主链已从 Go 基础与工程能力，逐步进入 HTTP/API、业务分层、SQL 与 migration、事务一致性、外部依赖、资源级访问控制、运行时生命周期、缓存、可观测，以及分层测试边界。
...
go run ./cmd/learning-content verify \
  --release-dir content/learning/releases/m1-first-slice-v31 \
  --web-dist web/dist
```
