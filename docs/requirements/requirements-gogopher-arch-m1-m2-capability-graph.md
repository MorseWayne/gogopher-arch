...
截至 `m1-first-slice-v8`，运行时已经发布 M1-01、M1-02、M1-03、M1-04、M1-05、M1-07、M1-08、M1-09 八个能力节点。第二批 M1-02、M1-04、M1-05、M1-08 均具备完整闭环：
...
每个独立评估只产生单一能力的 Evidence，且有同能力、不同题材的 review Activity；首次验证后可直接进入延迟复习调度。M1-08 还增加了可执行的导出 API 消费者测试和 doc comment AST 验收，文档不再只是阅读建议。

为保持已发布 Attempt 的冻结定义可重放，早期节点仍保留其 v1 前置关系和综合诊断评估。M1-08 v1 暂以当前可达的 M1-05、M1-07 为硬前置；M1-06 发布后通过新 Capability 版本加入接口/泛型硬前置，不回写 v1。下一内容批次按生产顺序进入 M1-11、M1-12、M1-13。
