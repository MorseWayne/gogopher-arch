...
10. M2-05：已完成 project schema 练习、checks schema 独立评估与 alerts schema 延迟变式。
11. M2-06：已完成事务 runner 练习、run 完成事务独立评估与 alert acknowledgement 延迟变式。
12. M2-07：已完成受控 HTTP client 策略练习、external probe 独立评估与 alert webhook 延迟变式。
13. M2-08：已完成 API key 认证练习、project 资源级授权独立评估与 alert rule 删除延迟变式。
14. M2-09：已完成逆序 lifecycle stack 练习、gocheck-hub 服务启停独立评估与 alert worker 延迟变式。
15. M2-10：已完成有界重试策略练习、gocheck-hub 持久化检查 worker 独立评估与 alert delivery worker 延迟变式。
16. M2-11：已完成 TTL 与 negative cache 练习、gocheck-hub project cache-aside 独立评估与 alert rule cache 延迟变式。
17. M2-12：已完成请求关联与低基数遥测练习、gocheck-hub 可观测和健康边界独立评估与 alert API 延迟变式。
18. M2-13：已完成手动时钟练习、gocheck-hub 分层测试独立评估与 alert API 延迟变式。
19. M2-14：已完成发布计划门禁练习、gocheck-hub 交付流水线独立评估与 alert worker 异题复验。
20. M2-15：已完成固定 Miniflux 调用链练习、category API 独立 patch 与后台 Retry-After 异题复验。
21. M2-16：完成 `gocheck-hub` 毕业项目。
...
截至 `m1-first-slice-v33`，运行时已经发布 M1-01 至 M1-14、M2-01 至 M2-15 共二十九个能力节点。M1-01 已调整为从亲手完成第一个 Go 程序开始的引导、练习、评估、复习闭环；M2 后端阶段已经从标准库 HTTP、稳定 API 契约和业务分层进入 SQL、schema migration、事务一致性、外部 HTTP、访问控制、运行时生命周期、可观测、分层测试、可复现交付与固定版本开源项目修改：
...
| M2-05 | project 关系约束与查询索引 | gocheck-hub checks schema 与 forward-only migration | gocheck-hub alerts schema 迁移 |
| M2-06 | transaction runner 提交与回滚 | gocheck-hub run 完成、幂等重放与版本冲突 | gocheck-hub alert acknowledgement 事务迁移 |
| M2-07 | 受控 `http.Client` 与超时策略 | gocheck-hub external probe 调用、取消与失败映射 | gocheck-hub alert webhook 交付迁移 |
| M2-08 | API key digest 与恒定时间认证 | gocheck-hub project 资源级授权与密钥隔离 | gocheck-hub alert rule 删除授权迁移 |
| M2-09 | LIFO 资源关闭栈 | gocheck-hub 配置、依赖初始化与 graceful shutdown | gocheck-hub alert worker 启停迁移 |
| M2-10 | 有界指数退避策略 | gocheck-hub 持久化检查 worker、背压与恢复 | gocheck-hub alert delivery worker 迁移 |
| M2-11 | TTL 与 negative cache | gocheck-hub project cache-aside、一致性与降级 | gocheck-hub alert rule cache 迁移 |
| M2-12 | 请求关联与低基数遥测 | gocheck-hub structured log、metrics、liveness/readiness | gocheck-hub alert API 可观测迁移 |
| M2-13 | 并发安全 manual clock | gocheck-hub unit/handler/PostgreSQL 分层测试 | gocheck-hub alert API 测试迁移 |
| M2-14 | 非 root runtime 与完整 gate 集合 | gocheck-hub 容器、CI、依赖安全与 migration 门禁 | gocheck-hub alert worker 交付流水线迁移 |
| M2-15 | 固定 Miniflux API/后台调用链 | category title 校验训练 patch | feed refresh Retry-After 策略迁移 |

每个独立评估只产生单一能力的 Evidence，且有同能力、不同题材的 review Activity；首次验证后可直接进入延迟复习调度。M1-06 通过 AST、隐藏替身和跨类型隐藏测试共同验证使用方最小接口与实用泛型，避免只靠测试偶然通过。M1-08 v2 将 M1-06 加入硬前置，并继续使用可执行的导出 API 消费者测试和 doc comment AST 验收。M1-10 要求把失败测试、breakpoint、Vet 与 `alloc_space` profile 连接成诊断小结，不能只提交修复代码。M1-12 增加只在 Submit 装载的私有 `race_test` 资产与 `race` 评测阶段，普通功能测试通过不能替代真实 `go test -race` 证据；私有源码和事件详情继续由服务端隔离。M1-14 增加受策略约束的新建/删除文件能力，并以 `new_project` 证据上下文要求学习者从空工作区交付完整 module；gocheck 独立评估和 endpointaudit 延迟变式都经过真实 sandbox、隐藏测试、Vet、Race Detector 与证据生成验证。M2-01 从 `httptest` 内存请求切片渐进到真实 loopback listener：隐藏测试分别验证 method-aware route、middleware context 传播、四类 server timeout 和活动请求期间的 `Shutdown` 等待；AST 规则要求学习者补充至少三个命名 case 的表格测试，解释规则要求串起 `ServeMux`、`context`、`timeout` 与 `Shutdown`。M2-02 继续沿用同一项目上下文，但把重点收敛到传输契约：严格拒绝未知字段与多余 JSON，验证 URL 和数值边界，把非法输入、领域冲突与内部故障稳定映射为 400、409 和 500，同时禁止内部错误泄漏；7 天后用不同 DTO、领域错误和 URL 约束的 alert API 复验迁移能力。M2-03 在同一项目中把 transport、use case 和 storage 拆成可替换边界：业务消费方拥有最小 storage 接口，transport 拥有调用业务所需接口，composition root 通过 constructor 选择 concrete 实现；隐藏测试验证每层行为和整链组装，AST 同时验证接口归属、constructor 调用与 learner 表格测试，解释规则要求说明依赖方向。延迟复验把相同原则迁移到 alert 业务切片。M2-05 把 M2-04 的 SQL 调用落到关系模型：两步 forward-only migration 分别创建约束和增加复合索引，生产查询与 EXPLAIN 查询保持同一形状；私有测试通过编译期嵌入 learner SQL，在删除 held-out 源码后仍能验证外键、CHECK、唯一性、禁止破坏性升级及索引列顺序，并以 AST 和解释规则补齐测试与设计证据。M2-06 用 serializable transaction 把 idempotency key 登记、重复读取和 expected version 条件更新放入同一原子边界；标准库私有 driver 实际并发两个不同命令，验证一次提交、一次稳定冲突，并覆盖重复 key 不重复写入、写故障回滚、事务调用 AST 与方案解释。M2-07 要求每个外部请求继承调用方 `context`，无论状态码都 `Close` response body，以 `io.LimitReader` 限制读取，并把 429、4xx、5xx 映射为稳定错误；隐藏 `RoundTripper` 和可跟踪 body 同时验证请求契约、取消传播、资源生命周期、大体限制和默认不 retry，变式用 POST JSON webhook 复验迁移。M2-08 在构造边界将 API key 转为 SHA-256 digest，并通过 `subtle.ConstantTimeCompare` 对等长值执行认证；隐藏 store 与 audit logger 替身验证未认证请求不触及资源、跨租户访问与不存在共用 404、canonical ID 在查询前拒绝，且 token、资源和底层错误不进入日志或响应；延迟变式将同一契约迁移到 alert rule 删除。M2-09 把 configuration 校验放在所有 I/O 之前，并在 composition root 显式初始化 database、handler 与 server；隐藏替身记录启动、Serve/Run、Shutdown 与 Close 的精确时序，验证 constructor 失败回滚、服务自发失败清理、SIGTERM/Context 取消后的有界关闭，以及 server 或 worker 先于其依赖资源的 reverse order。M2-10 让固定数量的领取循环直接形成 backpressure，不把持久化积压复制进无界内存队列；Store/Queue 通过 lease 和 owner 支持进程重启后的重领，通过 idempotency key 抑制重复副作用，并把成功、临时故障、永久故障和尝试耗尽分别映射到 Ack、Retry 与 Fail。隐藏并发替身验证领取上限、取消后 join、重复任务和 replacement owner，AST 与解释规则要求明确持久化恢复边界。M2-11 以 cache-aside 为主线，区分 cold miss、positive hit 和 negative hit，使用不同 TTL 控制陈旧与穿透窗口；Cache 故障降级到 source of truth，同 key flight 合并并发 miss 且 waiter 独立响应 context。写路径先提交真相再 invalidation，隐藏替身分别验证 Source 失败不失效，以及 Delete 失败返回已提交实体和包装错误，避免伪装成事务回滚。

M2-12 用显式 response recorder 捕获隐式 200、显式状态、响应字节与 duration；有效 request ID 同步进入 response header、request context 和 structured log，非法或超长 ID 在边界重建。指标只接收规范 method、构造时注入的 route template 与 status class，资源 ID、query、request ID 和依赖错误都不能成为高基数 label。隐藏替身分别验证 correlation、低基数聚合、安全失败与确定时钟；liveness 完全不触及依赖，readiness 才继承请求 context 检查关键依赖并把任意内部错误收敛为稳定 503。

M2-13 冻结生产 slice，只允许学习者编辑 service、handler 与 PostgreSQL 三类测试：unit test 用 deterministic clock、ID source 和 Store fake 控制时间与副作用；handler test 用 `httptest` 和 Creator fake 固定 JSON/status contract；integration test 固定使用 pgx，从 `TEST_DATABASE_URL` 读取 DSN，并通过 `sql.Open`、`PingContext`、`t.Cleanup` 与 `ExecContext` 建立隔离 fixture。隐藏测试验证冻结 slice 与 JSON fixture，AST 验证测试分层入口，真实 Sandbox 运行快速层；显式 `integration` build tag 加载只读 pgx driver，仓库回归把两份真实解答连接到真实 PostgreSQL。

M2-14 把发布要求收敛成同一组可执行契约：multi-stage Docker build 在固定 Go builder 中生成静态二进制，最小 Alpine runtime 只复制产物并切换到 non-root 用户；Makefile 与 GitHub Actions 同时覆盖 fmt、vet、test、race、govulncheck、migration 和 image gate。gocheck-hub assessment 校验 forward-only migration 的命名、唯一序号与输入顺序，alert worker 变式进一步验证 SHA-256 manifest。私有测试在 Sandbox 删除 held-out 源码前通过 `go:embed` 封存 learner 的 Dockerfile、Makefile 与 workflow，运行期只执行隔离后的测试二进制；AST 验证 parser 与 table cases，解释规则要求串起镜像边界、依赖安全和数据升级策略。

M2-15 固定 Miniflux v2.3.2 commit `51f2e0d8199ea8fa305081f6e175bba64b0ef94b` 与 Go 1.26.0，先从真实 route、scheduler、storage、worker 和 reader 符号建立 API/后台调用链。独立评估保留 upstream module、路径、model、SPDX 与 Apache-2.0 LICENSE，只以 consumer-owned store seam 隔离 PostgreSQL，要求 create/update 共用 Unicode title 规范化、长度和重复检查。异题复验迁移到 feed refresh 的 `Retry-After`：用注入时钟解析 delta-seconds/HTTP-date、拒绝过去值并施加 caller cap，handler 仅在 429 上采用延迟。平台协议与 schema 同时接受历史 `go1.25` 和新 `go1.26` task，CI、builder 与 Sandbox 固定 1.26，历史 release 继续可重放。

为保持已发布 Attempt 的冻结定义可重放，早期 release 仍保留其原始前置关系和评测定义。`m1-first-slice-v9` 中的 M1-08 v1 未被回写；v10 通过 M1-08 v2 显式加入 M1-06 硬前置；v11 新增 M1-14；v13 以 Activity/Task v2 前向增加综合提交的冷缓存预算至 45 秒；v14 新增 M2-01；v15、v16 前向调整初学入口；v17 新增 M2-02；v18 新增 M2-03；v19 新增 M2-04 的 SQL driver 测试；v20 新增 M2-05 migration 契约；v21 新增 M2-06 的并发事务闭环；v22 新增 M2-07 的外部 HTTP 失败闭环；v23 新增 M2-08 的访问控制闭环；v24 新增 M2-09 的三个 Activity/Task、启动回滚与有界逆序关闭私有时序测试以及 AST/解释规则；v25 新增 M2-10 的三个 Activity/Task、持久化 lease、幂等去重、固定并发背压与故障恢复私有测试以及 AST/解释规则；v26 新增 M2-11 的三个 Activity/Task、positive/negative TTL、cache outage 降级、并发 miss 合并、写后失效与部分成功私有测试以及 AST/解释规则；v27 新增 M2-12 的三个 Activity/Task、request ID 关联、structured log、低基数 metrics、响应观测、liveness/readiness 私有测试以及 AST/解释规则；v28 冻结初版 M2-13；v29 前向补齐 pgx driver registration 与 `go.sum`；v30 将受影响的 assessment/review Activity 与 Task 升为 v2，并让启动 Registry 继续验证、索引全部冻结 release，但只向数据库注册 current release 与 Attempt 实际引用的历史 release；未引用的 superseded release 继续可供冻结回放但不会污染数据库定义历史，已引用的冲突 release 仍会显式阻止启动。整个过程均未回写任何历史 release，并继续保留真实 PostgreSQL gate；v31 将首课 guided Activity/Task 前向升为 v8，把冷缓存 Build/Test/Vet 预算提高到 15 秒、Submit 提高到 45 秒，避免容器内首次编译挤占动作预算；v32 新增 M2-14 的三个 Activity/Task、最小非 root runtime、CI/Makefile gate 对齐、forward-only migration 与 checksum 私有契约，以及 AST/解释规则；v33 新增 M2-15 的三个 Go 1.26 Activity/Task、固定 Miniflux 来源与许可证、API/后台调用链、Unicode category patch、确定性 Retry-After 私有契约，以及 AST/解释规则。下一内容批次进入 M2-16 的后端服务完整交付。
