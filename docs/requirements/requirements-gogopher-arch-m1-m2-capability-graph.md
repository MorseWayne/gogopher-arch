...
20. M2-15：迁移到固定版本 Miniflux，完成真实代码导览与修改。
...
截至 `m1-first-slice-v32`，运行时已经发布 M1-01 至 M1-14、M2-01 至 M2-14 共二十八个能力节点。M1-01 已调整为从亲手完成第一个 Go 程序开始的引导、练习、评估、复习闭环；M2 后端阶段已经从标准库 HTTP、稳定 API 契约和业务分层进入 SQL、schema migration、事务一致性、外部 HTTP、访问控制、运行时生命周期、可观测、分层测试与可复现交付边界：
