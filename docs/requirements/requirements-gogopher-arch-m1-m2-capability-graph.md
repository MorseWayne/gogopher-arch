...
18. M2-13、M2-14：补分层测试、构建和交付。
19. M2-15、M2-16：完成 Miniflux 迁移和 `gocheck-hub` 毕业项目。
...
截至 `m1-first-slice-v27`，运行时已经发布 M1-01 至 M1-14、M2-01 至 M2-12 共二十六个能力节点。M1-01 已调整为从亲手完成第一个 Go 程序开始的引导、练习、评估、复习闭环；M2 后端阶段已经从标准库 HTTP、稳定 API 契约和业务分层进入 SQL、schema migration、事务一致性、外部 HTTP、访问控制、运行时生命周期、持久化后台任务与可观测边界：
