...
4. M1-06、M1-10：在真实代码中深化接口、泛型和调试。
5. M1-14：完成并试学 `gocheck`。
6. M2-01 至 M2-06：打通 HTTP、业务和数据库主线。
...
截至 `m1-first-slice-v9`，运行时已经发布 M1-01、M1-02、M1-03、M1-04、M1-05、M1-07、M1-08、M1-09、M1-11、M1-12、M1-13 十一个能力节点。M1-02、M1-04、M1-05、M1-08 与并发批次均具备完整闭环：
...
每个独立评估只产生单一能力的 Evidence，且有同能力、不同题材的 review Activity；首次验证后可直接进入延迟复习调度。M1-08 增加了可执行的导出 API 消费者测试和 doc comment AST 验收。M1-12 增加只在 Submit 装载的私有 `race_test` 资产与 `race` 评测阶段，普通功能测试通过不能替代真实 `go test -race` 证据；私有源码和事件详情继续由服务端隔离。

为保持已发布 Attempt 的冻结定义可重放，早期节点仍保留其 v1 前置关系和综合诊断评估。M1-08 v1 暂以当前可达的 M1-05、M1-07 为硬前置；M1-06 发布后通过新 Capability 版本加入接口/泛型硬前置，不回写 v1。下一内容批次按生产顺序进入 M1-06、M1-10，再汇入 M1-14 完整程序交付。
