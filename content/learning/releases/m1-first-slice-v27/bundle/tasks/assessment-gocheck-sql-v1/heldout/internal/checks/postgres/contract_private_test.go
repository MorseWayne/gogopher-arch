...
import (
	...
	"gocheckhub/internal/checks/postgres"
)
...
func openDB(t *testing.T, state *driverState) *sql.DB {
	t.Helper()
	register.Do(func() { sql.Register("m204-checks", testDriver{}) })
	current = state
	db, err := sql.Open("m204-checks", "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPoolContract(t *testing.T) {
	...
	config := postgres.PoolConfig{MaxOpen: 7, MaxIdle: 3, MaxLifetime: time.Minute, MaxIdleTime: time.Second}
	if err := postgres.ConfigurePool(db, config); err != nil {
		t.Fatal(err)
	}
	if db.Stats().MaxOpenConnections != 7 {
		t.Fatal("MaxOpen not configured")
	}
	if err := postgres.ConfigurePool(db, postgres.PoolConfig{}); err == nil {
		t.Fatal("accepted invalid pool")
	}
	if _, err := postgres.NewRepository(nil); err == nil {
...
func TestContextQueriesAndArguments(t *testing.T) {
	...
	if err := repository.Create(context.Background(), checks.Check{ID: "c-1", Target: "api"}); err != nil {
		t.Fatal(err)
	}
...
func TestRowsLifecycleAndIterationErrors(t *testing.T) {
	...
	if err != nil || len(values) != 2 || values[1].Target != "worker" {
...
func TestSingleRowErrorsStable(t *testing.T) {
	...
	db := openDB(t, state)
	repository, _ := postgres.NewRepository(db)
	if _, err := repository.Find(context.Background(), "missing"); !errors.Is(err, checks.ErrNotFound) {
		t.Fatalf("missing error=%v", err)
	}
	state.values = [][]driver.Value{{"c-bad", nil}}
	if _, err := repository.Find(context.Background(), "bad"); err == nil || errors.Is(err, checks.ErrNotFound) {
		t.Fatalf("scan error=%v", err)
	}
	state.values = [][]driver.Value{{"c-3", "cron"}}
	value, err := repository.Find(context.Background(), "c-3")
	if err != nil || value.Target != "cron" {
...
func TestQueryCancellationPropagates(t *testing.T) {
	...
	cancel()
	if err := repository.Create(ctx, checks.Check{ID: "c", Target: "api"}); !errors.Is(err, context.Canceled) {
